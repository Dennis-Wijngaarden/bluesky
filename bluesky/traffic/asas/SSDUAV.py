...
def initializeSSD(asas, ntraf):
    ...
    #asas.inconf       = np.zeros(ntraf, dtype=bool) Commented out because it overwrites already existing array
...
def constructSSD(asas, traf, priocode = "RS1"):
    ...
    alpha = np.zeros(len(ind1))

    # In LoS the VO can't be defined, act as if dist is on edge
    for i in range(len(ind1)):
        hsepm_max = max(hsepm[ind1[i]], hsepm[ind2[i]])
        if dist[i] < hsepm_max:
            dist[i] = hsepm_max
        ...
        alpha[i] = np.arcsin(hsepm_max / dist[i])
    ...
    x1 = np.zeros(len(ind1))
    x2 = np.zeros(len(ind1))
    y1 = np.zeros(len(ind1))
    y2 = np.zeros(len(ind1))
    for i in range(len(ind1)):
        x1[i] = (sinqdr[i] + cosqdrtanalpha[i]) * 2 * max(vmax[ind1[i]], vmax[ind2[i]])
        x2[i] = (sinqdr[i] - cosqdrtanalpha[i]) * 2 * max(vmax[ind1[i]], vmax[ind2[i]])
        y1[i] = (cosqdr[i] - sinqdrtanalpha[i]) * 2 * max(vmax[ind1[i]], vmax[ind2[i]])
        y2[i] = (cosqdr[i] + sinqdrtanalpha[i]) * 2 * max(vmax[ind1[i]], vmax[ind2[i]])
        ...
        if asas.inconf[i]:
            ...
            if len(ind) == 0:
                ...
                for j in range(np.shape(i_other)[0]):
                    ...
                    hsepm_max = max(hsepm[i], hsepm[ind[j]])
                    if dist[ind[j]] > hsepm_max:
                        ...
                        leg = 1.1 * vmax[i] / np.cos(beta) * np.array([1,1,1,0])
                        # Angles of darttip
                ...
                if len(ARV) == 0:
                    ...
                    if priocode == "RS2" or priocode == "RS9" or priocode == "RS6" or priocode == "RS3" or priocode == "RS4":
                        ...
                        if priocode == "RS2" or priocode == "RS9" or priocode == "RS6":
                            ...
                            part = pyclipper.scale_to_clipper(tuple(map(tuple, 1.1 * vmax[i] * xyp)))
                            pc2.AddPath(part, pyclipper.PT_SUBJECT, True)
                        elif priocode == "RS3":
                            # Small ring
                            xyp = (tuple(map(tuple, np.flipud(xyc * min(vmax[i],gs_ap[i] + 0.1)))), tuple(map(tuple , xyc * max(vmin[i],gs_ap[i] - 0.1))))
